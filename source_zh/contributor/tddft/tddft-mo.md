...
其中 $\kappa_c$ 为库仑耦合因子，由 `xlet` 决定（限制性参考：`'S'` 单重态 2、`'T'` 三重态 0；非限制参考以单位权重 1 耦合两个自旋扇区，`'R'` 为该非自旋适配路径的通用标记）。交换系数对普通杂化泛函为 $(c_x, 0)$，即 eq.5 的第二项消失。eq.2–3 与 eq.4–5 各为一次 DGEMV 与一次 DGEMM 链；fxc 贡献由 `fxc_matvec` 补充 (见下文)。B 块交换的指标排序 $(ib|aj)$ 由同一对 `ov_exch` 张量配合转置后的振幅实现，此处不赘述。
