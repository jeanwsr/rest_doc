# ADR-0001：并行 BLAS 线程调度

- **状态**：已接受
- **日期**：2026-08-13
- **适用范围**：REST 本体 (crate `rest`) 的全部模块 (SCF、post-SCF、GW、微扰、TD 等)，不限于特定模块。线程控制函数由 crate `rest_tensors` 提供实现；使用 RSTSR 张量运算时的处理见第 4 节。
- **作者**：祝震予 (ajz34@outlook.com)

本决定对 `rest` 的新增代码具有约束力；存量代码存在尚未遵循之处，将在重构中逐步对齐，本 ADR 不逐一列出。
本决定所述机制的最终形式、含全局线程初始化，随 rest_tensors PR #13 同时给出。

## 决定

- REST 目前 **OpenMP 编译的 OpenBLAS** 作为 BLAS 后端；本 ADR 仅对该情形有约束。其他数学库目前是实验性支持 (见第 5 节备注)。
- 在并发 (rayon) 线程内部调用 BLAS 前，调用 `omp_set_num_threads_wrapper(1)`；该函数是 thread-safe 与 thread-local 的，因此无需保存/恢复线程数。
- 使用 RSTSR 的矩阵乘法与 einsum 时，无需手动设置线程数 (见第 4 节)。
- 主线程初始化时，与 rayon 全局线程池的构建一起，调用一次 `omp_set_num_threads_global_wrapper` 设置全局的 BLAS 线程数；输入卡 `num_threads` 选项覆盖 `OPENBLAS_NUM_THREADS` 等环境变量的设置 (见 2.3 节)。
- pthreads 并行 OpenBLAS 的支持已废止 (见 3.2 节)。

基于 OpenMP 并行的多线程 OpenBLAS，REST 可以同时处理
- 普通多线程 BLAS/LAPACK 计算 (以下简称 BLAS)；
- 多线程并发线程内部的单线程 BLAS 计算。

对于普通多线程 BLAS 计算，普通地调用即可。

对于多线程并发、单线程 BLAS 计算任务，为了防止线程超支 (thread oversubscription)，我们需要在每个并发线程中，设置 BLAS 的线程数为 1。设置方法示例如下 (基于 rayon 的并发)：
...
`omp_set_num_threads_wrapper` 应当是 **thread-safe 与 thread-local** 的。对于 OpenMP 编译的 OpenBLAS 的情形，`omp_set_num_threads_wrapper` 即对 `omp_set_num_threads` 的简单封装。

除此之外，在主程序中，我们需要使用线程不安全的函数 `omp_set_num_threads_global_wrapper` 以设置全局的 BLAS 线程数。该函数仅在主线程中调用一次，与 rayon 全局线程池一起设置。

上述两个线程控制函数定义于 `rest_tensors::matrix::matrix_blas_lapack`，在 crate rest 中经 `extern crate rest_tensors as tensors` 以 `tensors::matrix_blas_lapack::*` 的路径引用。两者的性质与实际调用如下：
...
- 上述设计用于 rest_tensors 张量，或直接对 BLAS 作 FFI 调用。对于 RSTSR 的情况，请参考第 4 节 (RSTSR 的处理模式)。
- 如果只能保证 thread-safe 而不能保证 thread-local (即只能作全局设置)，则需要在并发前确认线程数、并发后恢复线程数。参考第 3.2 节 (pthreads 并行的 OpenBLAS 情形)；3.2 节的做法在 thread-local 的情形也适用但冗余。

## 1. 设计动机

电子结构计算大量地需要使用 BLAS 函数。REST 目前主要使用 OpenBLAS 作为 BLAS 库。

BLAS 函数有两种调用情景。一种调用情形较为简单，即普通的矩阵乘法或矩阵分解。
...
### 2.3 主线程初始化线程池时使用 `openblas_set_num_threads`

一些细节参考下面的 3.1 节 (`openblas_set_num_threads` 存在 thread-safety 问题)。我们可以在主线程中调用 `openblas_set_num_threads` 来设置全局的 BLAS 线程数，但不应该在并发线程中调用该函数。
...
实践中，这一初始化由 `omp_set_num_threads_global_wrapper` 完成，它会同时调用 `openblas_set_num_threads` 与 `omp_set_num_threads` (见前文的对照表)。

## 3. 注意事项与已废止的设计决定

### 3.1 `openblas_set_num_threads` 存在 thread-safety 问题
...
### 3.2 已废止：pthreads 并行的 OpenBLAS 支持
...
### 3.3 部署验证：确认 OpenBLAS 为 OpenMP 构建

本 ADR 的线程控制方案依赖 OpenMP 构建的 OpenBLAS。若实际链接到 pthreads 构建的 OpenBLAS，`omp_set_num_threads_wrapper` 的设置将静默失效 (仅 `omp_set_num_threads_global_wrapper` 中的全局设置仍然有效)：程序一般仍能运行，但并发批量 BLAS 计算会发生线程超支，即退化为 3.2 节的情形。
...
为达到这种设计目的，有必要使用 thread-local 的线程控制。RSTSR 在运行时通过 `openblas_get_parallel()` 识别 OpenBLAS 的并行方式并选择线程控制函数：OpenMP 构建使用 `omp_set_num_threads` (thread-local)；pthreads 构建则退化为 `openblas_set_num_threads` (全局的设置与恢复，即 3.2 节的模式)，无法兑现 thread-local 的语义。因此，对于下述用法，REST 规范要求 OpenMP 并行的 OpenBLAS。
