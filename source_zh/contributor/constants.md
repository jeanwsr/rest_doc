...
2. 若文件已有 `use crate::constants::{...};`，请将新常量**并入已有的 `use` 语句**，而非新增一行。

3. **不要在代码中硬编码物理常量数值**（如 `0.52917721092`、`27.2114`、`1822.8885` 等）。应统一从 `constants` 模块引用。

4. 若所需常量尚不存在，请在 `src/constants/mod.rs` 中新增，并附上来源注释（例如 CODATA 年份及对应的 NIST 数值）。
