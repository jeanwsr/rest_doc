...
在上例中，第一个 O 原子和第一个 H 原子被固定（标记为 `0`），其余原子自由优化。
...
[ctrl.geometric_pyo3]
    maxiter =                   500
    convergence_energy =        1.0e-7
    convergence_grms =          1.0e-4
    convergence_gmax =          3.0e-4
    convergence_drms =          1.0e-3
    convergence_dmax =          1.5e-3
    trust =                     0.2
    tmax =                      0.5
    subfrctor =                 2
    reset =                     false
