...
- `start_diis_cycle`: 适用于 "diis" 方法，指定从第几轮迭代开始启用 DIIS 加速收敛，缺省值为 2。
- `num_max_diis`: 适用于 "diis" 方法，指定 DIIS 子空间的维度，即使用最近多少轮次的 Fock 矩阵误差向量进行计算，缺省值为 8。
...
对于能隙较小或金属性体系，自洽场计算中轨道占据数在 HOMO/LUMO 附近发生剧烈跳变，导致密度矩阵在迭代间大幅波动，DIIS 等方法难以有效加速收敛。针对此类问题，REST 程序引入了smearing功能，通过引入展宽参数$\sigma$，将原本整数的轨道占据数替换为平滑的分数占据分布，从而稳定自洽场迭代过程。

当启用 smearing 后，体系的能量 $E(\sigma)$ 包含了由分数占据引入的熵贡献。程序会同时跟踪三个能量：

1. **$E(\sigma)$**：当前温度下的原始自洽场能量。
2. **$E_{\text{free}} = E(\sigma) - \sigma S$**：扣除熵贡献后的自由能，程序以此作为自洽收敛的能量判据，保证迭代过程稳定。
3. **$E_0 = E(\sigma) - \frac{1}{2} \sigma S$**：对 $\sigma \to 0$ 极限的一阶线性外推，是最终推荐使用的能量结果。

在 REST 中，通过以下关键词控制分数轨道占据：

- `smear`：取值 String，目前支持：
  - `"fermi"`：Fermi-Dirac 展宽，占据数分布为 $f_i = \frac{1}{1 + \exp((\epsilon_i - \mu) / \sigma)}$
  - `"gaussian"`：Gaussian 展宽，占据数分布为 $f_i = \frac{1}{2} \cdot \operatorname{erfc}\frac{\epsilon_i - \mu}{\sigma}$ 
  其中$\mu$为化学势，在程序中通过二分法确定。Gaussian 展宽范围比 Fermi-Dirac 更宽，平滑性更强。
  
  缺省值为 None（不开启展宽）。

- `smear_sigma`：取值 f64，展宽参数 $\sigma$，单位为 Hartree。$\sigma$ 越大，占据数分数化程度越高，收敛越快但引入的熵误差越大。对于小分子体系，典型取值范围为 0.01–0.05 Ha。缺省为 None。

- `smear_anneal`：取值 bool，是否启用在自洽场迭代中对 $\sigma$ 做指数退火（annealing）。缺省为 false。设置为 `true` 后，$\sigma$ 在迭代过程中按指数衰减：

  $$\sigma(\text{iter}) = \sigma_{\text{start}} \cdot \left( \frac{\sigma_{\text{min}}}{\sigma_{\text{start}}} \right)^{t}, \quad t = \min\left(1, \frac{\text{iter} - \text{start}}{\text{length}}\right).$$

  其中iter为当前自洽场迭代轮次，start为退火起始轮次，等于 `start_diis_cycle`，length为退火持续的轮数。退火窗口为 `max_scf_cycle` 的前半段——前半段 $\sigma$ 从初始值指数衰减至下限 $\sigma_{\text{min}}$，后半段保持恒定下限 $\sigma_{\text{min}}$，供 DIIS 稳定收敛。退火完成后，由于 $\sigma$ 已衰减至较小的 $\sigma_{\text{min}}$，$E(\sigma)$ 与零温极限已非常接近。计算结束时，程序同时输出 $E(\sigma)$、 $E_{\text{free}}$ 和 $E_0$ 三个能量值供用户参考。

- `smear_sigma_min`：取值 f64，退火过程中 $\sigma$ 的下限值，单位为 Hartree。缺省值为 $\max(\text{smear\_sigma} \times 0.01, 0.001)$。需要特别指出的是，$\sigma$ 不应退火至零：对于高对称性过渡金属等具有严格简并轨道的体系，$\sigma \to 0$ 会导致占据数在简并轨道间发生乒乓（ping-pong）振荡而无法收敛。保留非零下限可确保简并流形的平滑平均占据。若体系简并程度高、退火后仍不收敛，可手动加大该值。

一个使用 Fermi-Dirac 展宽配合退火的典型输入如下：
...
关键词 `outputs` 以字符串数组的形式声明所需的性质计算选项，它的主要功能是用于生成标准的波函数文件，以方便被后续的带有丰富波函数分析和可视化功能的程序调用，例如 Multiwfn、Qbics-MolStar 等。关于后处理程序的调用，可以参考 [后处理](./post_process.md) 文档。
关于 `outputs` 关键词的具体说明如下：
