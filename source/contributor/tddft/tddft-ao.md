...
## Function `prepare_ao_data`

Function path: `ri_tddft::tddft::prepare_ao_data`

Builds the AO-mode `TDDFTData`. The MO-specific members (`fxc`, `ri_*`) are `None`; the AO-side members are:
...
| `fxc_eff` | raw kernel table $f^{\mathrm{xc}}_{\alpha\beta}(g)$ (singlet factor included) | $(n_\mathrm{grid}, n_\mathrm{var}, n_\mathrm{var})$ | weights not multiplied; see below |
| `den_type` | `RHO` / `SIGMA` | — | determines $n_\mathrm{var}$ |
| `grid_batch` | grid-batching switch | — | |
| `ao` / `ao_grad` | AO values on grid | $(n_\mathrm{basis}, n_\mathrm{grid})$ | allocated only for the dim ≤ 15 dense path |
...
**Spin-channel selection** (driven by `tddft_spin`):
...
**fxc driver selection** `tddft_fxc_driver` (AO mode only, default `"dm"` = the eq.1–3 path above):
...
with $\Lambda^0_{ia} = \psi_i\psi_a$ and $\Lambda^{d+1}_{ia} = \partial_d\psi_i\,\psi_a + \psi_i\,\partial_d\psi_a$. The output is MO amplitudes directly — no $[n_\mathrm{basis},n_\mathrm{basis},m]$ intermediates, no `contract_back`.
...
i.e. the identity matrix is used as the trial-vector block, and one pass through `ao_a_kernel_block` (folded J + K + batched fxc + contraction back to the MO basis) yields all kernel columns at once, plus the diagonal. The mode dispatch is encapsulated in `tddft::build_a`/`build_b` (the MO branch loops unit columns through `a_matvec`), so the solver code is mode-agnostic.
